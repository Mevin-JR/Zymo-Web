  ...
  const handlePlaceSelect = () => {
    if (autocomplete) {
      ...
      if (placeDetails.geometry) {
        ...
        setAddress(address.length > 2 ? `${address[0]}, ${address[1]}, ${address.at(-2)}` : address);
  ...
  return (
    <>
      ...
      <div className="bg-[#303030] rounded-full p-3 mx-auto mb-6 w-full max-w-md sm:w-[60%] md:max-w-xl lg:max-w-2xl">
        <h1 className={`text-white text-center text-md transition-opacity duration-500 ${fade ? "opacity-0" : "opacity-100"}`}>
        ...
        <div className="flex justify-center gap-8 mb-6">
          {["rent", "subscribe", "buy"].map((tab) => (
            <button
              ...
              className={`text-lg ${activeTab === tab ? "text-white border-b-2 border-gray-200" : "text-gray-400"}`}
            >
