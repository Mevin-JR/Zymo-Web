export const calculateFreeKM = (rateBasis, tripDuration) => {
  ...
  export const fetchMyChoizeCars = async ( CityName, formattedPickDate, formattedDropDate, tripDuration) => {
    ...
    try {
      ...
      const groupedCars = {};
      mychoizeData.SearchBookingModel
    ...
    Object.values(groupedCars[key].rateBasisFare).forEach((fare) => {
      ...
      return Object.values(groupedCars).map((car) => ({
        ...car,
        fare: `₹${Math.min(...car.all_fares)} - ₹${Math.max(...car.all_fares)}`,
      }));
    } catch (error) {
